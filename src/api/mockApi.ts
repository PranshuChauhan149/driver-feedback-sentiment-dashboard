...
// Mock Data
export const FEEDBACK_TAGS: Record<string, FeedbackTag[]> = {
  driver: [
...
export const mockDrivers: Driver[] = Array.from({ length: 50 }, (_, i) => {
  ...
  let status: Driver['status'] = 'excellent';
  ...
  return {
    ...
    name: driverNames[i % driverNames.length] + (i >= 10 ? ` ${Math.floor(i / 10)}` : ''),
...
export const mockFeedback: Feedback[] = Array.from({ length: 200 }, (_, i) => {
  const entityTypes = ['driver', 'trip', 'app', 'marshal'] as const;
...
export const mockApi = {
  ...
  getSentimentSummary: async (): Promise<SentimentSummary> => {
    await delay(300);
    const positive = mockFeedback.filter((f) => f.sentiment === 'positive').length;
    const neutral = mockFeedback.filter((f) => f.sentiment === 'neutral').length;
    const negative = mockFeedback.filter((f) => f.sentiment === 'negative').length;
    ...
    const driversAboveThreshold = mockDrivers.filter((d) => d.averageScore >= 4.0).length;
    const driversBelowThreshold = mockDrivers.filter((d) => d.averageScore < 2.5).length;
  ...
  submitFeedback: async (data: FeedbackSubmission): Promise<void> => {
    await delay(800);
    // Simulate success
